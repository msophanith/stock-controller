...
const ExportFAB = dynamicImport(
  () =>
    import("@/components/dashboard/export-fab").then((mod) => mod.ExportFAB),
  { ssr: false },
);

interface DashboardClientProps {
  initialProducts: Product[];
  initialTodaySales: StockMovement[];
  initialStats?: {
...
export default function DashboardClient({
  ...
  initialTodaySales,
  initialStats,
  recentProducts: serverRecentProducts,
  alertProducts: serverAlertProducts,
}: DashboardClientProps) {
  // Use initial data in the queries to avoid loading state on first load
  const { data: products = initialProducts } = useProducts({
    initialData: initialProducts,
    staleTime: 0, // Ensure it refetches in the background to get the full list
  });
  const { data: todaySales = initialTodaySales } = useTodaySales({
    initialData: initialTodaySales,
  });
  const activityLog = useProductStore((s: any) => s.activityLog);

  const todayStats = useMemo(() => {
    const sales = todaySales.filter((m) => m.type === "OUT");
    const returns = todaySales.filter((m) => m.type === "RETURN");
  ...
  const stats = useMemo(() => {
    // If we have server-calculated stats and we're using initial data, use them
    if (initialStats && products === initialProducts) {
      return {
        ...initialStats,
        alertProducts: serverAlertProducts || [],
        recentProducts: serverRecentProducts || [],
